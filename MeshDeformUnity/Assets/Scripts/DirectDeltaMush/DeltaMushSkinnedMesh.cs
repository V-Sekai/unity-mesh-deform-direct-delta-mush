...
public class DeltaMushSkinnedMesh : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            verticesCB =
                new ComputeBuffer(mesh.vertices.Length, 3 * sizeof(float));
            normalsCB =
                new ComputeBuffer(mesh.vertices.Length, 3 * sizeof(float));
            weightsCB =
                new ComputeBuffer(mesh.vertices.Length,
                    4 * sizeof(float) + 4 * sizeof(int));
            verticesCB.SetData(mesh.vertices);
            normalsCB.SetData(mesh.normals);
            weightsCB.SetData(mesh.boneWeights);

            adjacencyCB =
                new ComputeBuffer(adjacencyMatrix.Length, sizeof(int));
            var adjArray = new int[adjacencyMatrix.Length];
            Buffer
                .BlockCopy(adjacencyMatrix,
                0,
                adjArray,
                0,
                adjacencyMatrix.Length * sizeof(int));
            ...
            outputCB[0] =
                new ComputeBuffer(mesh.vertices.Length, 6 * sizeof(float));
            outputCB[1] =
                new ComputeBuffer(mesh.vertices.Length, 6 * sizeof(float));
            outputCB[2] =
                new ComputeBuffer(mesh.vertices.Length, 6 * sizeof(float));
            ...
            computeShader
                .SetInt("AdjacentNeighborCount", adjacencyMatrix.GetLength(1));
            ...
            computeShader
                .GetKernelThreadGroupSizes(laplacianKernel,
                out threadGroupSizeX,
                out threadGroupSizeY,
                out threadGroupSizeZ);
        ...
        prefilteredBoneWeights =
            SmoothFilter
                .distanceWeightedLaplacianFilter(mesh.vertices,
                prefilteredBoneWeights,
                adjacencyMatrix);
...
#region Delta Mush implementation
    ...
    GetSmoothDeltas(
        Vector3[] vertices,
        int[,] adjacencyMatrix,
        Func<Vector3[], int[,], Vector3[]> filter,
        int iterations
    )
    {
    ...
    void UpdateMeshOnCPU()
    {
        ...
        if (usePrefilteredBoneWeights)
        {
            ...
            for (int i = 0; i < mesh.vertexCount; i++)
            {
                ...
                for (int b = 0; b < boneCount; b++)
                vertexMatrix[n] +=
                    boneMatrices[b][n] * prefilteredBoneWeights[i, b];

                deformedMesh.vertices[i] =
                    vertexMatrix.MultiplyPoint3x4(vs[i] - deltaV[i]);
                deformedMesh.normals[i] =
                    vertexMatrix.MultiplyVector(ns[i] - deltaN[i]);
    ...
    void UpdateMeshOnGPU()
    {
        ...
        for (int i = 0; i < iterations; i++)
        {
            ...
            computeShader
                .SetBool("DeltaPass", lastIteration && !disableDeltaPass);
            computeShader.SetBuffer(laplacianKernel, "Input", outputCB[i % 2]);
            computeShader
                .SetBuffer(laplacianKernel, "Output", outputCB[(i + 1) % 2]);
